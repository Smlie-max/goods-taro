...
class BargainOrderPreview extends Component {
  ...
  render() {
    ...
    return (
      <View className='BargainOrderPreview'>
        <Menu />
        {
          ...
          info.merch_list.map((item) => {
            return (
              <View className='order-block' key={item.id}>
                ...
                <View className='get-way' onClick={this.showDeliveryMask.bind(this)}>
                  <Text>配送</Text>
...
export default connect(mapStateToProps, mapDispatchToProps)(BargainOrderPreview);
